...
import { MQTTAuthenticate, MQTTClient, MQTTPublishPacket, MQTTSubscribePacket,MQTTAuthorizePublish,MQTTAuthorizeSubscribe,MQTTPreConnect,MQTTPublished, MQTTConnectPacket  } from 'src/common/mqtt';
...
export class BrokerService implements OnModuleInit {
  ...
  @MQTTAuthenticate()
  authHandler(client: MQTTClient, username?: string, password?: string): boolean {
  ...
  authorizePublish(
    ...
    console.log(`Authorizing publish for client: ${client.id} to topic: ${packet.topic}`);
  ...
  onPublished(
    ...
    console.log(`Message published to topic: ${packet.topic} by client: ${client.id}`);
